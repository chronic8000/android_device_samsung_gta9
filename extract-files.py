...
blob_fixups: blob_fixups_user_type = {
    ...
    ('vendor/lib64/libmtkcam_stdutils.so', 'vendor/lib64/hw/android.hardware.camera.provider@2.6-impl-mediatek.so'): blob_fixup()
        # Changed from libutils-v32.so to libutils-v36.so
        .replace_needed('libutils.so', 'libutils-v36.so'),
    'vendor/lib64/sensors.camera.light.so': blob_fixup()
        # Changed from libcamera2ndk_vendor-v35.so to libcamera2ndk_vendor-v36.so
        .replace_needed('libcamera2ndk_vendor.so', 'libcamera2ndk_vendor-v36.so'),
    ...
    'vendor/bin/hw/android.hardware.media.c2@1.2-mediatek-64b': blob_fixup()
        # Assuming v36 minimum for A16
        .add_needed('libstagefright_foundation-v36.so')
        .replace_needed('libavservices_minijail_vendor.so', 'libavservices_minijail.so'),
    'vendor/lib64/hw/audio.primary.mediatek.so': blob_fixup()
        # Assuming v36 minimum for A16
        .add_needed('libstagefright_foundation-v36.so')
        # Changed from libalsautils-v31.so to libalsautils-v36.so
        .replace_needed('libalsautils.so', 'libalsautils-v36.so'),
